...
Reset:	CLI
	...
	ANDI R16, ~(1<<WDRF)
	OUT MCUSR, R16
	IN R16, WDTCSR	; Пишем 1 в WDCE and WDE
	ORI R16, (1<<WDCE) | (1<<WDE)
	OUT WDTCSR, R16
	;Записываем новое значение предделителя времени задержки
	LDI R16, (0<<WDP3) |(1<<WDP2) | (1<<WDP1) | (0<<WDP0) | (1<<WDE) | (1<<WDIE)	; Предделитель на 1 секунду
	OUT WDTCSR, R16
...
WDT_OVERFLOW:
	...
	IN R16, WDTCSR
	ORI R16, (1<<WDIE)	;Включаем прерывание по WDT,
	OUT WDTCSR, R16	; если не включить на следующем цикле произойдёт сброс
