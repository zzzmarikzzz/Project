...
;avrdude -p t24 -c arduino-ft232r -P ft0 -U flash:w:Timer_Tstat.hex:i -v
...
.def     FailureCNT=R5	;Счётчик сбоев чтения температуры
...
.def     Flag=R24	;Регистр флагов
		.equ	 Mode=0		;Режим сравнения времени
		.equ	 Dot_Flag=1	;флаг точек для моргания
.def     TemperatureLow=R20
.def     TemperatureHigh=R21
.equ	 TemperatureDest=23
...
.equ	Light_DDR=DDRA
.equ	Light_PORT=PortA
.equ	Light=PA1	; Управление светом на порте PA1

.equ	HEAT_DDR=DDRA
.equ	HEAT_PORT=PortA
.equ	Heat=PA0	; Управление нагревом на порте PA0
...
.equ	OW_DQ	= PB2				; Ножка порта, где висит 1-Wire
...
RESET:	
	...
	SBI Light_DDR, Light	;настройка порта Освещения
	CBI Light_PORT, Light	;Выключение Освещения
	...
	ORI R16, 0<<MUX5|0<<MUX4|0<<MUX3|0<<MUX2|1<<MUX1|0<<MUX0	;Для кнопок используется ADC2 (PA2)
	OUT ADMUX, R16
	...
	LDI Temp, high(7812)		;инициализация компаратора 7812 - 0.5 сек
	OUT OCR1AH,Temp
	LDI Temp, low(7812)
	OUT OCR1AL,Temp
...
TIM1_COMPA:
	...
	LDI Temp, 1<<Dot_Flag
	EOR Flag, Temp		;Инвертируем Dot_Flag 
	Rcall CHECK_DOT
...
Light_ON: SBI Light_PORT, Light ; Включаем свет
	rjmp Bezdel

Light_Off: CBI Light_PORT, Light ; Выключаем свет
...
ReadTerm:
	...
	RCALL OWWriteByte
	CLR R16
	RCALL OWReadByte	;Читаем 3 байта
	MOV TemperatureLow,R16
	RCALL OWReadByte
	MOV TemperatureHigh,R16
	RCALL OWReadByte
	PUSH R16
	RCALL OWReset
	LDI R16,0xCC		;Scip ROM
	...
	RCALL OWWriteByte
	POP R16
	CPI R16, 0x4B	;Третий байт вседа должен читаться как 0x4B
	BRNE BAD	;Если данные с термометра прочитались не корректно - используем старые
	
	POP R16
	POP R16
	CLR FailureCNT		;сбрасываем счетчик сбоев
	RCALL t_convert
	RJMP ChoosingHeat
	
BAD: INC FailureCNT		;увеличиваем счетчик сбоев
	MOV R16, FailureCNT
	CPI R16, 10
	BRLO PC+3		;переход на 3 комманды вперед (пропустить 2)
	LDI R16, 10		;если счетчак досчитал до 10 - не даём ему рости дальше
	MOV FailureCNT, R16
	POP TemperatureLow	;записываем в Старые Значения
	POP TemperatureHigh
ChoosingHeat:	MOV R16, FailureCNT
	CPI R16, 10		; Если градусник не отвечает 10 раз подряд - выключаем обогрев
	BRSH HeatOFF
	
	LDI R17, TemperatureDest
	LDI R16, 4
	RCALL CPTerm
	BRLO MayBeNeedHeat
HeatOFF:	CBI HEAT_PORT, Heat	; Выключаем обогрев
...
;| На входе:	целая часть в TemperatureHigh текушей температуры
;|		дробная часть в TemperatureLow текушей температуры
...
CPTermLO:	SEC	;Если TemperatureHigh.TemperatureLow меньше R17.R16
	CLZ
	RET

CPTermSH:	CLC	;Если TemperatureHigh.TemperatureLow больше R17.R16
	CLZ
	RET
;|---------------------------------------------------------------------------
;|                               END
...
;| На входе: Младший байт в TemperatureLow, Старший байт в TemperatureHigh
;| На выходе: дробная часть в TemperatureLow, Целая часть со знаком в TemperatureHigh (старший бит)
;|---------------------------------------------------------------------------
t_convert: PUSH R16
	PUSH R17
	SBRS TemperatureHigh,7	;Если число положительное, то переходим к преобразованию дробной части
	rjmp IfPositiv
	CLR R16
	CLR R17
	SUB R16, TemperatureLow	;Переводим из дополнительного кода
	SBC R17, TemperatureHigh
	MOV TemperatureLow, R16
	MOV TemperatureHigh, R17
	ORI TemperatureHigh,1<<3	;ставим знак "-"

IfPositiv:
	MOV R16, TemperatureLow
ANDI R16, 0b11110000
	SWAP R16
	SWAP TemperatureHigh
	ANDI TemperatureHigh, 0b11110000
	OR TemperatureHigh,R16	;Склеиваем целые части младшего и старшего байтов в один байт

	ANDI TemperatureLow,0b00001111	;Преобразуем дробную часть
	mov R16, TemperatureLow	;Нужно умножить на 10, для этого:
	lsl R16		;Умножаем на 2
	lsl TemperatureLow
	lsl TemperatureLow
	lsl TemperatureLow		;Умножаем на 8
	ADD TemperatureLow,R16	;Складываем и умножение на 10 готово
	ANDI TemperatureLow, 0b11110000
	SWAP TemperatureLow	;Делим на 16
	CPI TemperatureHigh, 1<<7	;Проверка на -0, если -0.0 надо убрать минус
	BRNE end_t_convert
	TST TemperatureLow		;Проверка на -0.0
	BRNE end_t_convert
	CLR TemperatureHigh
